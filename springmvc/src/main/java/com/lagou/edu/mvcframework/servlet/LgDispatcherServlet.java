    ...
    private void initHandlerMapping() {
        ...
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                //  增加方法的用户权限信息
                List<String> security = handler.getSecurity();
                LagouSecurity methodSecurityAnno = method.getAnnotation(LagouSecurity.class);
                LagouSecurity classSecurityAnno = aClass.getAnnotation(LagouSecurity.class);
                if (methodSecurityAnno != null){
                    String[] value = methodSecurityAnno.value();
                    List<String> list = Arrays.asList(value);
                    security.addAll(list);
                }
                if (classSecurityAnno != null){
